...
from raillabel.format._attribute_type import AttributeType
...
def test_object_data_pointers__sensor():
    ...
    assert set(object_data_pointers.keys()) == set(
        ["rgb_middle__bbox__person", "lidar__bbox__person"]
    )
...
def test_object_data_pointers__annotation_type():
    ...
    assert set(object_data_pointers.keys()) == set(
        ["rgb_middle__bbox__person", "rgb_middle__cuboid__person"]
    )
...
def test_object_data_pointers__one_frame_interval():
    ...
    assert object_data_pointers["rgb_middle__bbox__person"].frame_intervals == [FrameInterval(0, 1)]
...
def test_object_data_pointers__two_frame_intervals():
    ...
    frames = {
        0: build_frame(0, {object: [build_annotation("rgb_middle__bbox__person", object)]}),
        1: build_frame(1, {object: [build_annotation("rgb_middle__bbox__person", object)]}),
        8: build_frame(8, {object: [build_annotation("rgb_middle__bbox__person", object)]}),
    }
...
def test_object_data_pointers__multiple_objects_of_differing_type():
    ...
    assert train_object_data_pointers["lidar__bbox__train"].frame_intervals == [FrameInterval(1, 1)]
...
def test_object_data_pointers__multiple_objects_of_same_type():
    ...
    assert object1_data_pointers["lidar__bbox__person"].frame_intervals == [FrameInterval(0, 0)]

    object2_data_pointers = object2.object_data_pointers(frames)
    assert len(object2_data_pointers) == 1
    assert object2_data_pointers["lidar__bbox__person"].frame_intervals == [FrameInterval(1, 1)]
...
def build_object(type: str) -> Object:
    return Object(
        uid=str(uuid4()), name=f"{type}_{str(random.randint(0, 9999)).zfill(4)}", type=type
    )
